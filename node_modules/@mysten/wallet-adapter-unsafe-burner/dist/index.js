"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var __accessCheck = (obj, member, msg) => {
  if (!member.has(obj))
    throw TypeError("Cannot " + msg);
};
var __privateGet = (obj, member, getter) => {
  __accessCheck(obj, member, "read from private field");
  return getter ? getter.call(obj) : member.get(obj);
};
var __privateAdd = (obj, member, value) => {
  if (member.has(obj))
    throw TypeError("Cannot add the same private member more than once");
  member instanceof WeakSet ? member.add(obj) : member.set(obj, value);
};
var __privateSet = (obj, member, value, setter) => {
  __accessCheck(obj, member, "write to private field");
  setter ? setter.call(obj, value) : member.set(obj, value);
  return value;
};

// src/index.ts
var src_exports = {};
__export(src_exports, {
  UnsafeBurnerWalletAdapter: () => UnsafeBurnerWalletAdapter
});
module.exports = __toCommonJS(src_exports);
var import_ed25519 = require("@mysten/sui.js/keypairs/ed25519");
var import_client = require("@mysten/sui.js/client");
var import_wallet_standard = require("@mysten/wallet-standard");
var import_faucet = require("@mysten/sui.js/faucet");
var _client, _keypair, _account, _faucetUrl;
var UnsafeBurnerWalletAdapter = class {
  constructor(client, faucetUrl) {
    this.name = "Unsafe Burner Wallet";
    this.icon = "data:image/png;base64,";
    __privateAdd(this, _client, void 0);
    __privateAdd(this, _keypair, void 0);
    __privateAdd(this, _account, void 0);
    __privateAdd(this, _faucetUrl, void 0);
    this.signMessage = async (messageInput) => {
      const { bytes, signature } = await __privateGet(this, _keypair).signPersonalMessage(messageInput.message);
      return {
        messageBytes: bytes,
        signature
      };
    };
    this.signPersonalMessage = async (messageInput) => {
      const { bytes, signature } = await __privateGet(this, _keypair).signPersonalMessage(messageInput.message);
      return {
        bytes,
        signature
      };
    };
    this.signTransactionBlock = async (transactionInput) => {
      const { bytes, signature } = await transactionInput.transactionBlock.sign({
        signer: __privateGet(this, _keypair),
        client: __privateGet(this, _client)
      });
      return {
        transactionBlockBytes: bytes,
        signature
      };
    };
    this.signAndExecuteTransactionBlock = async (transactionInput) => {
      return await __privateGet(this, _client).signAndExecuteTransactionBlock({
        signer: __privateGet(this, _keypair),
        transactionBlock: transactionInput.transactionBlock,
        options: transactionInput.options,
        requestType: transactionInput.requestType
      });
    };
    this.on = () => {
      return () => {
      };
    };
    __privateSet(this, _keypair, new import_ed25519.Ed25519Keypair());
    __privateSet(this, _faucetUrl, faucetUrl);
    __privateSet(this, _client, client || new import_client.SuiClient({
      url: (0, import_client.getFullnodeUrl)("localnet")
    }));
    __privateSet(this, _account, new import_wallet_standard.ReadonlyWalletAccount({
      address: __privateGet(this, _keypair).getPublicKey().toSuiAddress(),
      chains: ["sui:unknown"],
      features: ["sui:signAndExecuteTransactionBlock", "sui:signTransactionBlock"],
      publicKey: __privateGet(this, _keypair).getPublicKey().toBytes()
    }));
    this.connecting = false;
    this.connected = false;
    console.warn(
      "Your application is currently using the `UnsafeBurnerWalletAdapter`. Make sure that this adapter is not included in production."
    );
  }
  async getAccounts() {
    return [__privateGet(this, _account)];
  }
  async connect() {
    this.connecting = true;
    try {
      (0, import_faucet.requestSuiFromFaucetV0)({
        host: __privateGet(this, _faucetUrl) ?? (0, import_faucet.getFaucetHost)("localnet"),
        recipient: __privateGet(this, _keypair).getPublicKey().toSuiAddress()
      });
    } catch (e) {
      console.warn(
        "Failed to request Sui from the faucet. This may prevent transactions from being submitted."
      );
      console.warn(e);
    }
    this.connecting = false;
    this.connected = true;
  }
  async disconnect() {
    this.connecting = false;
    this.connected = false;
  }
};
_client = new WeakMap();
_keypair = new WeakMap();
_account = new WeakMap();
_faucetUrl = new WeakMap();
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  UnsafeBurnerWalletAdapter
});
//# sourceMappingURL=index.js.map